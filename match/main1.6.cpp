...
bool comparen(float a, float b) {
    int aa = a*1000;
    int bb = b*1000;
    if(aa>=bb*0.9 && aa<=bb*1.1) 
        return true;
        return false;
...
void dfs_2() {
    ...
    Node head=nodeList[i];
        ...
        while (q!=nullptr) {
            ...
            d = dhead.nextdst;
            while(d!=nullptr) {
                if(d->dstId!=q->trace[1] && d->times[1]>q->times[1] && comparen(d->money[1],q->money[1])) {
...
void dfs_3() {
    ...
    Node head=nodeList[i];
        ...
        while (q!=nullptr) {
            ...
            d = dhead.nextdst;
            while(d!=nullptr) {                
                if(d->trace[2]!=q->trace[1] && d->dstId!=q->trace[1] && d->times[1]>q->times[1] && comparen(d->money[1],q->money[1])) {
                    ArcNode *o = new ArcNode(q->trace[1], d->dstId, q->times[1], q->money[1]);
                    o->add_trace(d->trace[1],d->times[1], d->money[1]);
            ...
            while(d!=nullptr) {
                ...
                if(flag){
                            if(d->dstId == q->trace[1]){
                                if (d->times[1]>q->times[q->trace[0]] && comparen(d->money[1],q->money[q->trace[0]])){
...
int main(){
    ...
    char* inputFilePath1 = (char*)"/root/yangwenxin/a/account.csv";
    char* inputFilePath2 = (char*)"/root/yangwenxin/a/transfer.csv";
    ...
    */
    char* outputFilePath = (char*)"/root/yangwenxin/a/result.csv"; 
